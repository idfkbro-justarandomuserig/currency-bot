# bot.py
# Based on the user-provided ".txt" file code
# Includes Lottery, Supporter/VIP commands, setjackpot command
# Corrected admin command structure and Range/Param syntax
import disnake
...
for name, placeholder_id in original_placeholders.items():
    if placeholder_id is not None and current_ids.get(name) == placeholder_id:
         logger.warning(f"Config Warning: {name} still set to original placeholder ({placeholder_id}).")
...
SHOP_OPEN_HOUR = int(os.getenv("SHOP_OPEN_HOUR", 10))
SHOP_OPEN_MINUTE = int(os.getenv("SHOP_OPEN_MINUTE", 0))
SHOP_CLOSE_HOUR = int(os.getenv("SHOP_CLOSE_HOUR", 21))
SHOP_CLOSE_MINUTE = int(os.getenv("SHOP_CLOSE_MINUTE", 0))
SHOP_OPEN_TIME = time(SHOP_OPEN_HOUR, SHOP_OPEN_MINUTE)
SHOP_CLOSE_TIME = time(SHOP_CLOSE_HOUR, SHOP_CLOSE_MINUTE)

# Gambling Settings
SLOT_EMOJIS = ["🍎", "🍊", "🍋", "🍉", "🍇", "🍓", "🍒", "⭐", "💎"]
SLOT_JACKPOT_EMOJI = "💎"
SLOT_JACKPOT_CONTRIBUTION = 0.10
DICE_WIN_MULTIPLIER = 5
REDBLACK_WIN_MULTIPLIER = 1.9
REDBLACK_COOLDOWN_SECONDS = 5
...
# Data File Paths
DATA_DIR = "data"
USER_DATA_FILE = os.path.join(DATA_DIR, "user_balances.json")
SHOP_ITEMS_FILE = os.path.join(DATA_DIR, "shop_items.json")
BOT_DATA_FILE = os.path.join(DATA_DIR, "bot_data.json")

if not DISCORD_BOT_TOKEN: logger.critical("FATAL: DISCORD_BOT_TOKEN not found."); exit(1)
os.makedirs(DATA_DIR, exist_ok=True)

# --- Data Persistence ---
user_data = {}
shop_items = {}
bot_data = {} # Holds jackpot pool, lottery pot, lottery tickets
        ...
        for user_id_str, data in loaded_data.items():
            try:
                ...
                if isinstance(data, int):
                    migrated_data[user_id] = {"balance": data, "savings": 0, "pin": None}; migration_needed = True
...
bot = commands.Bot(
    ...
    intents=intents,
    help_command=None,
    sync_commands_debug=True # Use the deprecated flag from user's original code
)
...
async def autosave_data(): logger.debug("Autosaving..."); save_user_data(); save_shop_items(); save_bot_data(); logger.debug("Autosave complete.")
@autosave_data.before_loop
async def before_autosave(): await bot.wait_until_ready(); logger.info("Starting autosave loop.")
...
async def on_ready():
    ...
    logger.info('------')
    if PLACEHOLDER_IDS_PRESENT: logger.warning("!!! Original placeholder IDs might still be in use. !!!")
    ...
    if not bot.retroactive_scan_done:
        logger.info("Starting retroactive message scan...")
                ...
                if permissions.read_message_history and permissions.view_channel:
                    ...
                    try:
                        async for message in channel.history(limit=SCAN_MESSAGE_LIMIT_PER_CHANNEL):
                            total_messages_scanned += 1; guild_message_count += 1; channel_message_count += 1
        ...
        logger.info(f"--------------------------")
        save_user_data()
    else: logger.info("Retroactive scan already done.")
...
async def notify_shopkeepers(interaction: disnake.Interaction, item_data: dict, payment_method: str) -> bool:
    ...
    if success_count > 0: logger.info(f"Notified {success_count} shopkeepers ({fail_count} failed)."); await interaction.followup.send(f"✅ Purchase initiated! {success_count} shopkeeper(s) notified.", ephemeral=True); return True
    else: logger.error(f"Failed to notify any shopkeepers."); await interaction.followup.send("❌ Failed to notify any shopkeepers.", ephemeral=True); return False
...
async def shop(interaction: disnake.ApplicationCommandInteraction):
    ...
    embed = disnake.Embed(title="🛒 Shop 🛒", description="Click to buy.\n", color=disnake.Color.blurple())
    if is_shop_open(): embed.description += "Shop: **OPEN**."; embed.color = disnake.Color.green()
    else: embed.description += f"Shop: **CLOSED**.\nReopens: {time_until_shop_opens()}."; embed.color = disnake.Color.dark_grey()
...
class ShopAdminCog(commands.Cog):
    ...
    async def cog_command_error(self, inter: disnake.ApplicationCommandInteraction, error: commands.CommandError):
        if isinstance(error, (commands.CheckFailure, commands.UserInputError, commands.NoPrivateMessage)): await inter.response.send_message(str(error), ephemeral=True)
        ...
        except Exception: pass # Ignore errors sending error response

    # --- Shop Admin Group (Corrected Structure) ---
    ...
    async def shopadmin_list(self, inter: disnake.ApplicationCommandInteraction):
        ...
        for item_id, item in sorted(shop_items.items(), key=lambda i: i[1].get('name', '').lower()):
             if items_in_page >= max_items: embeds.append(disnake.Embed(description=current_desc, color=disnake.Color.orange())); current_desc = ""; items_in_page = 0
             ...
             if len(current_desc) + len(line) > 4000: embeds.append(disnake.Embed(description=current_desc, color=disnake.Color.orange())); current_desc = ""; items_in_page = 0 # Force page break if too long
        ...
        # Send first page (pagination view not implemented here)
    ...
    async def shopadmin_remove(self, inter: disnake.ApplicationCommandInteraction, item_id: str):
        ...
        if item_id in shop_items:
            name = shop_items[item_id].get('name', '?')
            del shop_items[item_id]; save_shop_items()
    ...
    class ShopAddItemModal(disnake.ui.Modal):
        def __init__(self):
            components = [
                disnake.ui.TextInput(label="Name", placeholder="Item Name", custom_id="item_name", max_length=100, required=True),
    ...
    @shopadmin.sub_command(name="add", description="Add a new item to the shop via a modal.")
    ...
    # --- Admin Coins Group (Corrected Structure & Params) ---
    @commands.slash_command(name="admincoins", description="Manage user coin balances.")
    async def admincoins(self, inter: disnake.ApplicationCommandInteraction): pass

    @admincoins.sub_command(name="give", description="Give coins to a user's balance.")
    async def admincoins_give(self,
                              inter: disnake.ApplicationCommandInteraction,
                              user: disnake.Member,
                              amount: int = commands.Param(description="Amount to give.", ge=1) # Corrected
                             ):
    ...
    @admincoins.sub_command(name="take", description="Take coins from a user's balance.")
    async def admincoins_take(self,
                              inter: disnake.ApplicationCommandInteraction,
                              user: disnake.Member,
                              amount: int = commands.Param(description="Amount to take.", ge=1) # Corrected
                             ):
        if user.bot: await inter.response.send_message("❌ Cannot mod bot.", ephemeral=True); return
        udata = get_user_data(user.id);
        if not isinstance(udata["balance"], (int, float)): udata["balance"] = 0
    ...
    @admincoins.sub_command(name="set", description="Set a user's balance to a specific amount.")
    async def admincoins_set(self,
                             inter: disnake.ApplicationCommandInteraction,
                             user: disnake.Member,
                             amount: int = commands.Param(description="Exact balance.", ge=0) # Corrected
                            ):
        if user.bot: await inter.response.send_message("❌ Cannot mod bot.", ephemeral=True); return
        udata = get_user_data(user.id)
        udata["balance"] = amount
        logger.info(f"Admin {inter.author} set {user.id}'s balance to {amount}.")
        await inter.response.send_message(f"✅ Set {user.mention}'s balance to {amount:,}.", ephemeral=True, allowed_mentions=disnake.AllowedMentions.none())

    @admincoins.sub_command(name="setjackpot", description="Set the slots jackpot pool amount.")
    async def admincoins_setjackpot(self,
                                    inter: disnake.ApplicationCommandInteraction,
                                    amount: float = commands.Param(description="Jackpot pool amount.", ge=0.0) # Corrected
                                   ):
        load_bot_data()
...
@bot.slash_command(name="savings", description="Manage your savings account.")
...
async def gamble_slots(inter: disnake.ApplicationCommandInteraction, amount: int = commands.Param(description="Amount to bet.", ge=1)): # Corrected Param
    ...
    await inter.response.defer(ephemeral=True); load_bot_data()
    udata["balance"] -= amount
    # (Animation)
    initial_embed = disnake.Embed(title="🎰 Slots 🎰", description="❓ ❓ ❓", color=disnake.Color.dark_gold()).set_footer(text=f"Bet: {amount:,}")
    try: await inter.edit_original_message(embed=initial_embed)
    except Exception as e: logger.warning(f"Slots init fail: {e}"); udata["balance"] += amount; return # Refund
    ...
    for i in range(spin_count):
        ...
        spin_embed = disnake.Embed(title="🎰 Slots 🎰", description=" ".join(display_reels), color=disnake.Color.dark_gold()).set_footer(text=f"Bet: {amount:,} | Spinning...")
        try: await inter.edit_original_message(embed=spin_embed); await asyncio.sleep(0.6 - i*0.07)
        except Exception: break
    # (Result)
    reels = final_reels; result_embed = disnake.Embed(title="🎰 Slot Result 🎰", description=" ".join(reels)).set_footer(text=f"Bet: {amount:,}")
    ...
    result_embed.add_field(name="New Balance", value=f"{int(udata['balance']):,} coins", inline=True)
    ...
    try: await inter.edit_original_message(embed=result_embed)
    except Exception as e: logger.warning(f"Slots final fail: {e}")

@gamble_base.sub_command(name="dice", description="Guess the roll of a 6-sided die.")
async def gamble_dice(inter: disnake.ApplicationCommandInteraction,
                      guess: int = commands.Param(description="Your guess (1-6).", ge=1, le=6), # Corrected
                      amount: int = commands.Param(description="Amount to bet.", ge=1) # Corrected
                     ):
    user_id = inter.author.id; udata = get_user_data(user_id)
    if not isinstance(udata.get("balance"), (int, float)): udata["balance"] = 0
    if udata["balance"] < amount: await inter.response.send_message(f"❌ Insufficient balance.", ephemeral=True); return
    ...
    result = disnake.Embed(title=f"🎲 Dice: {dice_emoji}", footer=f"Bet:{amount:,}|Guess:{guess}")
    winnings = 0
    if guess == roll: winnings = amount * DICE_WIN_MULTIPLIER; udata["balance"] += winnings; result.description = f"🎉 Correct! Won **{winnings:,}**!"; result.color = disnake.Color.green()
    else: result.description = f"😥 Incorrect (was {roll})."; result.color = disnake.Color.red()
    if not isinstance(udata["balance"], (int, float)): udata["balance"] = 0
    result.add_field(name="New Balance", value=f"{int(udata['balance']):,} coins", inline=False)
...
async def gamble_redblack(inter: disnake.ApplicationCommandInteraction,
                          choice: str = commands.Param(description="Your choice.", choices=["red", "black"]), # Corrected
                          amount: int = commands.Param(description="Amount to bet.", ge=1) # Corrected
                         ):
    user_id = inter.author.id; udata = get_user_data(user_id)
    if not isinstance(udata.get("balance"), (int, float)): udata["balance"] = 0
    ...
    result = disnake.Embed(title=f"{emoji} R/B: {color} ({roll})", footer=f"Bet:{amount:,}|Choice:{choice.capitalize()}")
    winnings = 0; match = (choice == "red" and is_red) or (choice == "black" and not is_red)
    if match: winnings = int(amount*REDBLACK_WIN_MULTIPLIER); udata["balance"] += winnings; result.description = f"🎉 Correct! Won **{winnings:,}**!"; result.color = disnake.Color.red() if is_red else disnake.Color.black()
    else: result.description = f"😥 Incorrect (was {color})."; result.color = disnake.Color.dark_grey()
    if not isinstance(udata["balance"], (int, float)): udata["balance"] = 0
    result.add_field(name="New Balance", value=f"{int(udata['balance']):,} coins", inline=False)
...
async def lottery_buy(inter: disnake.ApplicationCommandInteraction, tickets: int = commands.Param(description="Num tickets.", ge=1, default=1)): # Corrected Param
...
async def supporter_nickname(inter: disnake.ApplicationCommandInteraction,
                             new_nickname: str = commands.Param(description="Nickname (blank=reset).", default=None, max_length=32)):
    ...
    except Exception as e: logger.error(f"Nickname fail: {e}"); await inter.response.send_message(f"❌ Nickname fail.", ephemeral=True)
...
async def vip_embed(inter: disnake.ApplicationCommandInteraction, message: str = commands.Param(description="Message content.", max_length=2000)):
    ...
    try: await inter.channel.send(embed=embed); logger.info(f"VIP {inter.author} sent embed."); await inter.response.send_message("✅ Embed sent!", ephemeral=True)
...
async def help_command(inter: disnake.ApplicationCommandInteraction):
    await inter.response.defer(ephemeral=True); embeds = []
    base = disnake.Embed(title=f"{bot.user.name} Help", color=disnake.Color.blurple()).set_footer(text="<> required, [] optional.")
    cmap = {}; pgroups = set()
    ...
    for cmd in bot.slash_commands:
        ...
        if isinstance(cmd, commands.SubCommandGroup): pass # Skip complex groups for now
        elif isinstance(cmd, commands.slash_core.SlashGroup):
             pgroups.add(name)
             for sub in cmd.children.values():
                 ps = fmt_params(sub.options); s = f"</{name} {sub.name} {ps}>".strip()
                 cmap[cat].append({"cmd_string": s, "description": sub.description or "...", "admin": adm, "supporter": sup, "vip": vip })
    ...
    await inter.followup.send(embeds=embeds[:10], ephemeral=True) # Send first 10
...
async def on_close():
    ...
    logger.info("Final save..."); save_user_data(); save_shop_items(); save_bot_data(); logger.info("Save complete.")
